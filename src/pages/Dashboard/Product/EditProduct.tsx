...
import { useGetSingleProductQuery, useUpdateSingleProductMutation, } from '../../../redux/features/product/productApi';
...
const UpdateProduct: React.FC = () => {
    ...
    const { register, handleSubmit, formState: { errors }, setValue } = useForm<TInputs>();
    useEffect(() => {
        if (!isLoading && !error && productData) {
            // Set default values using productData once it's available
            setValue("name", productData.data.name || "");
            setValue("price", productData.data.price || "");
            setValue("quantity", productData.data.quantity || "");
            setValue("category", productData.data.category || "");
            setValue("brand", productData.data.brand || "");
            setValue("compatibility", productData.data.compatibility || "");
            setValue("capacity", productData.data.capacity || "");
            setValue("condition", productData.data.condition || "");
            setValue("interfaceType", productData.data.interfaceType || "");
            setValue("color", productData.data.color || "");
            setValue("formFactor", productData.data.formFactor || "");
        }
    }, [productData]);
